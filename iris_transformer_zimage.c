...
#define ZI_BF16_SDPA_SEQ    1024      /* Prefer BF16 SDPA for seq >= 1024 (saves GPU memory) */
#define ZI_MAX_SHARDS       32

/* Suppress expected fallback warnings for known models (moodyMix, ZIT). */
static int zi_suppress_fallback_warnings = 0;

void zi_set_suppress_warnings(int suppress) {
    zi_suppress_fallback_warnings = suppress;
}
...
static float *zi_get_tensor(safetensors_file_t **files, int n_files,
    ...
    if (!zi_suppress_fallback_warnings)
...
static int zi_load_block(zi_block_t *block, safetensors_file_t **files,
    ...
    /* Attention weights - try both separate and fused formats */
    int has_fused_attention = 0;
    
    /* First try to load fused attention tensor (moodyMix format) */
    snprintf(name, sizeof(name), "%s.attention.qkv.weight", prefix);
    float *fused_qkv_weight = zi_get_tensor(files, n_files, name, mmap_f32_weights);
    
    if (fused_qkv_weight) {
        /* Fused attention found - split into Q, K, V */
        has_fused_attention = 1;
        
        /* Allocate separate tensors */
        block->attn_q_weight = malloc(dim * dim * sizeof(float));
        block->attn_k_weight = malloc(dim * dim * sizeof(float));
        block->attn_v_weight = malloc(dim * dim * sizeof(float));
        
        /* Copy from fused tensor [3*dim, dim] -> [dim, dim] each */
        memcpy(block->attn_q_weight, fused_qkv_weight + 0 * dim * dim, dim * dim * sizeof(float));
        memcpy(block->attn_k_weight, fused_qkv_weight + 1 * dim * dim, dim * dim * sizeof(float));
        memcpy(block->attn_v_weight, fused_qkv_weight + 2 * dim * dim, dim * dim * sizeof(float));
        
        printf("Loaded fused attention tensor for %s\n", prefix);
        
        /* Don't free fused_qkv_weight if using mmap */
        if (!mmap_f32_weights) {
            free(fused_qkv_weight);
        }
    } else {
        /* Load separate attention tensors (original format) */
    ...
    /* Attention output weight - try both naming conventions */
    ...
    if (!block->attn_out_weight) {
        /* Try moodyMix naming */
        snprintf(name, sizeof(name), "%s.attention.out.weight", prefix);
        block->attn_out_weight = zi_get_tensor(files, n_files, name, mmap_f32_weights);
    }

    /* QK norm - try both naming conventions */
...
zi_transformer_t *zi_transformer_load_safetensors(const char *model_dir,
    ...
    /* Load image embedder - try both original and moodyMix naming */
    ...
    if (!tf->x_emb_weight)
        tf->x_emb_weight = zi_get_tensor(files, n_files, "x_embedder.weight", mmap_f32_weights);
    ...
    if (!tf->x_emb_bias)
        tf->x_emb_bias = zi_get_tensor(files, n_files, "x_embedder.bias", mmap_f32_weights);
    ...
    /* Load final layer - try both original and moodyMix naming */
    ...
    if (!tf->final_layer.adaln_weight)
        tf->final_layer.adaln_weight = zi_get_tensor(files, n_files, "final_layer.adaLN_modulation.1.weight", mmap_f32_weights);
    ...
    if (!tf->final_layer.adaln_bias)
        tf->final_layer.adaln_bias = zi_get_tensor(files, n_files, "final_layer.adaLN_modulation.1.bias", mmap_f32_weights);
    ...
    if (!tf->final_layer.norm_weight)
        tf->final_layer.norm_weight = zi_get_tensor_optional(files, n_files, "final_layer.norm_final.weight", mmap_f32_weights);
    ...
    if (!tf->final_layer.linear_weight)
        tf->final_layer.linear_weight = zi_get_tensor(files, n_files, "final_layer.linear.weight", mmap_f32_weights);
    ...
    if (!tf->final_layer.linear_bias)
        tf->final_layer.linear_bias = zi_get_tensor(files, n_files, "final_layer.linear.bias", mmap_f32_weights);
