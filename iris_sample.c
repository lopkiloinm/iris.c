...
float *iris_sample_euler_zimage(void *transformer,
    ...
    float *z_prev = (float *)malloc(latent_size * sizeof(float));
    ...
    for (int step = 0; step < num_steps; step++) {
        ...
        /* Skip the final step where sigma_next→0 (timestep→1.0).
         * This extreme timestep causes GPU numerical instability and is
         * unnecessary — the model has already converged. ComfyUI workflows
         * also avoid this via end_at_step < num_steps. */
        if (fabsf(sigma_next) < 1e-4f) break;
        ...
        /* Save latent before this step in case we need to revert */
        iris_copy(z_prev, z_curr, latent_size);
        ...
        /* Check for GPU failure (all-zero or NaN output) */
        {
            float absmax = 0;
            int bad = 0;
            for (int i = 0; i < latent_size && !bad; i++) {
                float v = model_out[i];
                if (v != v) { bad = 1; break; }
                float a = v < 0 ? -v : v;
            ...
            if (bad || absmax == 0.0f) {
                fprintf(stderr, "\nWarning: GPU returned invalid output at step %d, "
                        "reverting to step %d\n", step + 1, step);
                free(model_out);
                iris_copy(z_curr, z_prev, latent_size);
                break;
            }
        }
