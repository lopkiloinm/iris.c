...
case "$MODEL" in
    4b)
        ...
        QWEN3_REPO="BennyDaBall/Qwen3-4b-Z-Image-Engineer-V4"
        ;;
    4b-base)
...
# Use override repository if specified
if [ -n "$QWEN3_REPO" ]; then
    echo "Using Qwen3-4B repository: $QWEN3_REPO"
    QWEN3_BASE="https://huggingface.co/$QWEN3_REPO/resolve/main/safetensors"
    dl "$OUT/text_encoder/config.json" "$QWEN3_BASE/config.json"
    dl "$OUT/text_encoder/generation_config.json" "$QWEN3_BASE/generation_config.json"
    dl "$OUT/text_encoder/model.safetensors.index.json" "$QWEN3_BASE/model.safetensors.index.json"
...
" 2>/dev/null)
    ...
    for shard in $SHARDS; do
        dl "$OUT/text_encoder/$shard" "$QWEN3_BASE/$shard"
    done
    
    # Download tokenizer from Qwen3 repository safetensors directory
    dl "$OUT/tokenizer/added_tokens.json" "$QWEN3_BASE/added_tokens.json"
    dl "$OUT/tokenizer/chat_template.jinja" "$QWEN3_BASE/chat_template.jinja"
    dl "$OUT/tokenizer/merges.txt" "$QWEN3_BASE/merges.txt"
    dl "$OUT/tokenizer/special_tokens_map.json" "$QWEN3_BASE/special_tokens_map.json"
    dl "$OUT/tokenizer/tokenizer.json" "$QWEN3_BASE/tokenizer.json"
    dl "$OUT/tokenizer/tokenizer_config.json" "$QWEN3_BASE/tokenizer_config.json"
    dl "$OUT/tokenizer/vocab.json" "$QWEN3_BASE/vocab.json"
else
...
# tokenizer (skip if already downloaded from Qwen3 repo)
if [ -z "$QWEN3_REPO" ]; then
