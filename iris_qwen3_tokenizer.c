...
int *qwen3_tokenize_chat(qwen3_tokenizer_t *tok, const char *prompt,
    ...
    /* Optional system message: <|im_start|>system\n{system_prompt}<|im_end|>\n */
    if (system_prompt && system_prompt[0]) {
        if (total >= capacity) { capacity *= 2; tokens = realloc(tokens, capacity * sizeof(int)); }
        tokens[total++] = QWEN3_IM_START_ID;

        int *sys_role = qwen3_tokenize(tok, "system\n", &n, max_len - total);
        for (int i = 0; i < n && total < max_len; i++) {
            if (total >= capacity) { capacity *= 2; tokens = realloc(tokens, capacity * sizeof(int)); }
            tokens[total++] = sys_role[i];
        }
        free(sys_role);

        int *sys_tokens = qwen3_tokenize(tok, system_prompt, &n, max_len - total);
        for (int i = 0; i < n && total < max_len; i++) {
            if (total >= capacity) { capacity *= 2; tokens = realloc(tokens, capacity * sizeof(int)); }
            tokens[total++] = sys_tokens[i];
        }
        free(sys_tokens);
        ...
        int *sys_nl = qwen3_tokenize(tok, "\n", &n, max_len - total);
        for (int i = 0; i < n && total < max_len; i++) {
            if (total >= capacity) { capacity *= 2; tokens = realloc(tokens, capacity * sizeof(int)); }
            tokens[total++] = sys_nl[i];
        }
        free(sys_nl);
    }
