...
extern void zi_set_suppress_warnings(int suppress);

static int iris_load_zimage_transformer_if_needed(iris_ctx *ctx) {
    if (ctx->zi_transformer) return 1;  /* Already loaded */

    /* Suppress warnings for known models that use fallback naming */
    const char *model_dir = ctx->model_dir;
    if (strstr(model_dir, "moodymix") || strstr(model_dir, "zimage-turbo")) {
        zi_set_suppress_warnings(1);
    }
