 ...
 *
 * Implements Qwen3-4B model (BennyDaBall/Qwen3-4b-Z-Image-Engineer-V4 variant) 
 * for text encoding in Iris image generation.
...
/* Set default Qwen3-4B architecture values (BennyDaBall variant). */
...
float *qwen3_encode_text_ex(qwen3_encoder_t *enc, const char *prompt,
                              int *out_num_tokens) {
    return qwen3_encode_text_system(enc, prompt, out_num_tokens, NULL);
}
